...
class run_jobs(object):
    ...
    def running_SGE(self,vf,maxjob=100,is_debug=1):
        shell_work = 'perl %s/qsub-sge-backup.pl --resource vf=%s  --maxjob %d %s '%\
    ...
    def running_SGE_f4w(self,vf,maxjob=100,is_debug=1):
        #shell_work = 'perl %s/qsub-sge.pl --resource vf=%s  --maxjob %d %s '%\
        shell_work = 'perl %s/qsub-sge-YC2.pl --resource vf=%s  --maxjob %d %s '%\
             (self.bin, vf, maxjob, self.sh_work_file)
