...
import RNA_UMI.utils.module_StatInfo as m_Stat

class CountInfo(object):
    def __init__(self, s_idx, root_dir ):
#        self.l_sample = l_samp
#        self.l_file   = [ 
#            "%s/%s/%s.%s.xls" % (root_dir,samp,samp,ltype) for samp in l_samp
#        ]
...
#            infile = "%s/merge.%s.gene.xls"%(self.out_dir,self.ltype)
            infile = "%s/%s/%s.UMI_count.xls" % (self.root_dir ,self.s_idx, self.s_idx)
