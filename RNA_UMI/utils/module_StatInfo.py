...
#               self.GC_l_rate  = [ float(i) for i in f_1[8].split(';') ]
               self.ER_l_rate  = float(f_1[5])
               self.Q20_l_rate = float(f_1[6])
               self.Q30_l_rate = float(f_1[7])
               self.GC_l_rate  = float(f_1[8])
   
            self.N_remove  = l_line[3].split()[-1]
            self.Q_remove  = l_line[4].split()[-1]
            self.A_remove  = l_line[5].split()[2]
            self.A_trimed  = l_line[5].split()[-1]
         f_infile.close()
...
class SpikeIn(object):
   def __init__(self, file_HTS_SpikeIn,file_ERCC_info="/date/zhangshu/software/anaconda/lib/python2.7/site-packages/RNA_UMI/database/ercc.info.xls"):
