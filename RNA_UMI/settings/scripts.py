...
$bowtie_build_exe $dir_database/${ref}_ERCC92_RGC.refGene.fa                \\
    ...
    def s_001_add_UMI(self):
        l_sh_info     = []
        l_sh_info.append("samp=$1")
        l_sh_info.append("pl_exe=%s"       % (self.sftw_pl) )
        l_sh_info.append("pl_add_UMI=%s/new0930_add_UMI.pl" %(self.bin) )
        l_sh_info.append("dir=%s"       % (self.fastq  ))
        l_sh_info.append("""
$pl_exe $pl_add_UMI $dir/$samp.1.fq.gz $dir/$samp.2.fq.gz $dir/$samp.umi.fq.gz
        """)
        return l_sh_info

    def s_002_trim_TSO_polyA(self):
        l_sh_info     = []
        l_sh_info.append("samp=$1")
        l_sh_info.append("pl_exe=%s"       % (self.sftw_pl) )
        l_sh_info.append("pl_trim=%s/trim_TSO_polyA.pl" %(self.bin) )
        l_sh_info.append("dir=%s"       % (self.fastq  ))
        l_sh_info.append("""
$pl_exe $pl_trim $dir/$samp.umi.fq.gz $dir/$samp.umi.trim.fq.gz 0
        """)
        return l_sh_info
    ...
    def s_01_QC(self):
        ...
        l_sh_info.append("pl_exe=%s"       % (self.sftw_pl) )
        l_sh_info.append("pl_QC=%s/QC_plus_rm_primer-SE_truncatedRease.pl"  % (self.bin)     )
        l_sh_info.append("in_dir=%s"       % (self.dir_raw_data  ) )
        l_sh_info.append("out_dir=%s"      % (self.dir_clean_data) )
        l_sh_info.append("dir=%s"       % (self.fastq  ))
        l_sh_info.append("""""")
        l_sh_info.append("""
mv $dir/$samp.umi.trim.fq.gz $in_dir/$samp
...
if [ $data_dype == "2" ]
    ...
    def s_03_HTSeq_known(self):
        ...
        l_sh_info.append("umi_HTseq=%s/02_UMI_HTseq.py" % (self.bin))
        l_sh_info.append("TPM=%s/${ref}_tpm.py"         % (self.bin))
        l_sh_info.append("tophat_dir=%s"    % (self.dir_tophat))
...
rm  $tophat_dir/$samp_name/accepted_hits.umi.sort.sam                      \\
    $tophat_dir/$samp_name/accepted_hits.umi.sort.bam                      \\
...
#    $py_exe $cnt_py /dev/stdin >${out_dir}/repeat_count.bed
    ...
    def stat_02_SplitHTS(self):
        ...
        infile_RPKM= "%s/merge.dexseq_clean.RPKM.xls"        % (dir_HTS)
        out_Refseq = "%s/merge.dexseq_clean_refseq.gene.xls" % (dir_HTS)
        out_Refseq_RPKM="%s/merge.dexseq_clean_refseq.RPKM.xls"% (dir_HTS)
        out_lncRNA = "%s/merge.dexseq_clean_lncRNA.gene.xls" % (dir_HTS)
        out_lncRNA_RPKM= "%s/merge.dexseq_clean_lncRNA.RPKM.xls"% (dir_HTS)
        inNeo      = "%s/merge.dexseq_NeoRaw.gene.xls"       % (dir_HTS)
        inNeo_RPKM = "%s/merge.dexseq_NeoRaw.RPKM.xls"       % (dir_HTS)
        outNeo     = "%s/merge.dexseq_NeoPass.gene.xls"      % (dir_HTS)
        outNeo_RPKM= "%s/merge.dexseq_NeoPass.RPKM.xls" % (dir_HTS)
...
#grep -v -P '^lncGene|novoXLOC_' $infile_RPKM >$out_Refseq_RPKM
head -n 1 $infile >$out_lncRNA && grep -P '^lncGene' $infile >>$out_lncRNA
#head -n 1 $infile_RPKM  >$out_lncRNA_RPKM && grep -P '^lncGene' $infile_RPKM >>$out_lncRNA_RPKM

head -n 1 $inNeo >$outNeo
for i in `cut -f 1 $NeoINFO | uniq`;do grep -w $i $inNeo ;done >>$outNeo
#head -n 1 $inNeo_RPKM >$outNeo_RPKM
#for i in `cut -f 1 $NeoINFO | uniq`;do grep -w $i $inNeo_RPKM ;done >>$outNeo_RPKM 
        """)
        return l_sh_info
