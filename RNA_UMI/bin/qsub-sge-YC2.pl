...
=head1 Version
  ...
  fixed by Hezengquan hezengquan@genomics.org.cn
...
=head1 Exmple
  ...
  8.different resource
  perl qsub-sge.pl --queue all.q --resource vf=1.9G,vf=1.5G,vf=8G ./work.sh
...
##$Queue ||= "all.q";   # fixed by hezengquan 2009-09-17
$Interval ||= 10;
$Lines ||= 1;
$Maxjob ||= 100;
$Convert ||= 'yes';
$Resource ||= "vf=1.9G,p=2,io=0";    #################################revised by Lilin to set io 2016-8-7
...
while(<IN>){
	...
	if ($line_mark % $Lines == 0) {
        ...
        #push @Shell,"sh work_$Job_mark.sh";
        `chmod +x $Work_dir/work_$Job_mark.sh`;
...
while (@Shell) {
    ...
    ## pkurun -J XHL -A tangfuchou_g1 -p cn-long --qos tangfuchoucnl -N1 -n1 -c1 -o index.out -e split.err sh split.sh
	...
	#my $job_cmd = "qsub -cwd -V ";  ## -l h_vmem=16G,s_core=8 
    #please edit this to allocate resource for the program# By cyang
    #my $job_cmd = "pkurun -J XHL -A tangfuchou_g1 -p cn-long --qos tangfuchoucnl -N1 -n1 -c1 ";  ## -l h_vmem=16G,s_core=8 
    #my $job_cmd = "pkubatch -J XHL_MethGC -A tangfuchou_g1 -p cn-long --qos tangfuchoucnl -N1 -n1 -c1 ";  ## -l h_vmem=16G,s_core=8 
    my $job_cmd = "pkubatch -J XHL_RNA_UMI -A tangfuchou_g1 -p fat4way --qos tangfuchouf4w -N1 -n1 -c1 ";  ## -l h_vmem=16G,s_core=8 
	...
	#my $part_resource;
	#$job_cmd .= "-l $Resource " if(defined $Resource); ##set resource by Lilin 2016-8-7
	##warn $job_cmd;
	...
	for (my $i=0; $i<@Shell; $i++) {
        while (1) {
            if ($i < $Maxjob || run_count(\%Alljob,\%Runjob) < $Maxjob) {
				...
				print STDERR "execute: $job_cmd $Shell[$i]\n" if($Verbose);
