...
#            f_mergeRatio.close()
    ...
    def s10_mergeSample(self, depth=2):
        for query in self.l_query:
            make_dir([ self.dir_StatInfo_digital, self.s_idx, query ])
            mergeRatio = "%s/%s/%s/RatioMatrix.xls" % (self.dir_StatInfo_digital, self.s_idx, query)
            f_mergeRatio = open(mergeRatio, "w")
