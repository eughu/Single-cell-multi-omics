...
#        my_job.running_SGE(vf="vf=2G,p=1,io=0", maxjob=100, is_debug = self.is_debug)
        my_job.running_SGE(vf="vf=2G,p=1", maxjob=150, is_debug = self.is_debug)
...
#                )
            l_sh_work.append("sh %s %s %s" % \
                (sh_file, brief_name,self.ref)
            )
   

        my_job = m_jobs.run_jobs(sh_file, sh_work_file, l_sh_info, l_sh_work)
#        my_job.running_multi(cpu=4, is_debug = self.is_debug)
        my_job.running_SGE(vf="vf=25G,p=4", maxjob=150, is_debug = self.is_debug)
...
#                for query in self.l_query:
        ...
        my_job = m_jobs.run_jobs(sh_file, sh_work_file, l_sh_info, l_sh_work)
#        my_job.running_multi(cpu=10, is_debug = self.is_debug)
        my_job.running_SGE(vf="vf=10G,p=8", maxjob=100, is_debug = self.is_debug)
    ...
    def s04_lambdaMeth(self):
        ...
        my_job = m_jobs.run_jobs(sh_file, sh_work_file, l_sh_info, l_sh_work)
#        my_job.running_multi(cpu=10, is_debug = self.is_debug)
        my_job.running_SGE(vf="vf=8G,p=1", maxjob=150, is_debug = self.is_debug)
...
#        for brief_name in self.samInfo_pd_RNA['brief_name']:
        ...
        my_job = m_jobs.run_jobs(sh_file, sh_work_file, l_sh_info, l_sh_work)
#        my_job.running_multi(cpu=10, is_debug = self.is_debug)
        my_job.running_SGE(vf="vf=5G,p=1", maxjob=150, is_debug = self.is_debug)
...
#                   for query in self.l_query:
        ...
        my_job = m_jobs.run_jobs(sh_file, sh_work_file, l_sh_info, l_sh_work)
#        my_job.running_multi(cpu=10, is_debug = self.is_debug)
        my_job.running_SGE(vf="vf=8G,p=1", maxjob=250, is_debug = self.is_debug)
    ...
    def s07_NUC(self):
        ...
        my_job = m_jobs.run_jobs(sh_file, sh_work_file, l_sh_info, l_sh_work)
#        my_job.running_multi(cpu=10, is_debug = self.is_debug)
        my_job.running_SGE(vf="vf=6G,p=1", maxjob=250, is_debug = self.is_debug) 
...
#                    print brief_name, query, self.M_QuerySamChr[query][brief_name]
    ...
    def s09_mergeNUC_NDR_rm(self):
        sh_file      =  "%s/s09.mergeNUC_NDR_rm.sh"               % (self.scripts)
        sh_work_file =  "%s/s09.mergeNUC_NDR_rm_work.sh"          % (self.scripts)

        l_sh_info = self.s_09_mergeNUC_NDR_rm()
        l_sh_work = []
...
#            l_sh_work.append("sh %s %s %s"   % (sh_file, brief_name, self.ref))
        ...
        for samp in self.samInfo_pd_RNA['sample']:
           ...
           if ltype.split('_')[1] == "scBS":
              sc = 1
              l_sh_work.append("sh %s %s %s %d"  % (sh_file,brief_name, self.ref,sc))
...
#                        val = cnt_met/(cnt_met+cnt_umt)   ##digital by Lyn
#                        if val <= 0.1:  ##digital by Lyn
#                            val = 0     ##digital by Lyn
#                        if val >= 0.9:  ##digital by Lyn
#                            val = 1     ##digital by Lyn
#                        if 0.9 > val >0.1:  ##digital by Lyn
#                            val = "NA"      ##digital by Lyn
#                        val = str(val)  ##digital by Lyn
                    l_out.append(val)
...
#            db_file = "%s/%s/%s_lambda.fa.%s.geneBody_geneTSS.anno.bed" % (
#                self.Database_lyn, self.ref, self.ref, query
#            )
...
#        my_job.running_multi(cpu=8, is_debug = self.is_debug)
        my_job.running_SGE(vf="vf=3G,p=1,io=0", maxjob=100, is_debug = self.is_debug)
    ...
    def s12_NDR_flanking(self):
        ...
        my_job = m_jobs.run_jobs(sh_file, sh_work_file, l_sh_info, l_sh_work)
#        my_job.running_multi(cpu=10, is_debug = self.is_debug)
        my_job.running_SGE(vf="vf=3G,p=1", maxjob=150, is_debug = self.is_debug)
