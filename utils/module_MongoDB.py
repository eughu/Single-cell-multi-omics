...
class DbGene(object):
    ...
    def loadExp_from_file(self,
        ...
        self.pd_exp = pd.read_csv(infile_exp, sep="\t", index_col="tracking_id")
        self.__load_samInfo()
        self.__load_exp()
...
class DbTF(object):
    ...
    def __load_interval(self, infile_Bed):
        ...
        f_infile_Bed = open(infile_Bed, "r")
    ...
    def __NDR_gene_overlap(self, chrom, beg, end, ltype):
        ...
        try:
            ...
            for rec in record:
                ...
                if gene_id_find is not None:
                    score = score_onlyDist(input_beg, input_end, rec_beg, rec_end)
                    if score > best_score:
                        best_score = score
                        M_score_best['gene_id'] = gene_id_find['_id']
                        M_score_best['gene'] = gene
            
            if "gene" in M_score_best:
                self.info["gene_id"][gene] = M_score_best['gene_id']
                self.info['gene'].append(M_score_best['gene'])
