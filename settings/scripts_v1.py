...
$perl_exe /datd/lilin/bin/singleC_metLevel.mm9_GCH_WCG.lambda.pl $singleC_sam_dir/bam/$meth_name.sort.rmdup.lambda.bam.pileup >$singleC_sam_dir/bam/$meth_name.sort.rmdup.lambda.single5mC
...
if [ $sc == "0" ]
   ...
   $stat_3x_depth_exe -p 8 -d 3 $sites_ref $singleC_sam_prefix $query
fi
if [ $sc == "1" ]
...
if [ $sc == "0" ]
   then python $NDR_py $singleC_sam_prefix/$chrom.$query.bed.gz $singleC_sam_prefix/$query.genome.CT_total.xls $depth > $singleC_sam_prefix/$chrom.$query.NDR.bed && \\
...
if [ $sc == "1" ]
   then python $scNDR_py $singleC_sam_prefix/$chrom.$query.bed.gz $singleC_sam_prefix/$query.genome.CT_total.xls $depth > $singleC_sam_prefix/$chrom.$query.NDR.bed && \\
   $bgzip_exe -f $singleC_sam_prefix/$chrom.$query.NDR.bed                                                                                                   && \\
   $tabix_exe -f -p bed -s 1 -b 2 -e 3 $singleC_sam_prefix/$chrom.$query.NDR.bed.gz
fi
        """)
    ...
    def s_07_NUC(self):
        ...
        l_sh_info.append("NUC_tabix_16GC=%s/NDR_detect/NUC_tabix_16GC.pl" % (self.bin))
        l_sh_info.append("""
singleC_sam_prefix=$singleC_dir/$meth_name/singleC

if [ $sc == "0" ]
   then  python $NUC_py $singleC_sam_prefix/$chrom.GCA.GCC.GCT.bed.gz $singleC_sam_prefix/GCA.GCC.GCT.genome.3x.CT_total.xls 3 >$singleC_sam_prefix/$chrom.GCA.GCC.GCT.nucleosome.tmp     && \\
   perl $NUC_tabix_16GC $singleC_sam_prefix/$chrom.GCA.GCC.GCT.nucleosome.tmp $singleC_sam_prefix/$chrom.GCA.GCC.GCT.bed.gz  > $singleC_sam_prefix/$chrom.GCA.GCC.GCT.nucleosome.bed      && \\
   $bgzip_exe -f $singleC_sam_prefix/$chrom.GCA.GCC.GCT.nucleosome.bed    &&\\
   $tabix_exe -f -p bed -s 1 -b 2 -e 3 $singleC_sam_prefix/$chrom.GCA.GCC.GCT.nucleosome.bed.gz    && \\
   rm $singleC_sam_prefix/$chrom.GCA.GCC.GCT.nucleosome.tmp
fi
if [ $sc == "1" ]
   then  python $scNUC_py $singleC_sam_prefix/$chrom.GCA.GCC.GCT.bed.gz $singleC_sam_prefix/GCA.GCC.GCT.genome.3x.CT_total.xls 3 >$singleC_sam_prefix/$chrom.GCA.GCC.GCT.nucleosome.tmp     && \\
   perl $NUC_tabix_16GC $singleC_sam_prefix/$chrom.GCA.GCC.GCT.nucleosome.tmp $singleC_sam_prefix/$chrom.GCA.GCC.GCT.bed.gz  > $singleC_sam_prefix/$chrom.GCA.GCC.GCT.nucleosome.bed      && \\
   $bgzip_exe -f $singleC_sam_prefix/$chrom.GCA.GCC.GCT.nucleosome.bed    &&\\
   $tabix_exe -f -p bed -s 1 -b 2 -e 3 $singleC_sam_prefix/$chrom.GCA.GCC.GCT.nucleosome.bed.gz    && \\
   rm $singleC_sam_prefix/$chrom.GCA.GCC.GCT.nucleosome.tmp
...
python $mergeSC_py -r $sites_ref -c $query $singleC_sam_prefix
...
awk '{OFS="\\t";if ($4+$5>=1) print $1,$2,$3+1,$5/($4+$5)}' $singleC_sam_prefix/all.${query}.bed >$singleC_sam_prefix/all.${query}.bedGraph &&\\
...
ln -s $singleC_sam_prefix/all.${query}.bw  $TDFs_dir/$meth_name.${query}.bw

awk '{OFS="\\t";if ($4+$5>=1) print $1,$2,$4,$5,$5/($4+$5)}' $singleC_sam_prefix/all.${query}.bed >$singleC_sam_prefix/all.${query}.1x-srt
#ln -s $singleC_sam_prefix/all.${query}.1x-srt  /WPSnew/lilin/mouse_NOME_seq_select/StatInfo_steps/02.Tile/1x_singleC/$meth_name.${query}.1x-srt

#if [ $sc == "0" ]
#   then awk '{OFS="\\t";if ($4+$5==0) print $1,$2,$3,0,-1;if ($4+$5>0) print $1,$2,$3,$4+$5,$5/($4+$5)}' $singleC_sam_prefix/all.${query}.bed >$singleC_sam_prefix/all.${query}.digital.bed
#   $bgzip_exe -c $singleC_sam_prefix/all.${query}.digital.bed > $singleC_sam_prefix/all.${query}.digital.bed.gz  &&\\
#   $tabix_exe -f -p bed -s 1 -b 2 -e 3  $singleC_sam_prefix/all.${query}.digital.bed.gz
#$ucsc_dir/bedGraphToBigWig                                             \\
#    $singleC_sam_prefix/all.${query}.bedGraph                          \\
#    $sites_ref  $singleC_sam_prefix/all.${query}.digital.bw
#
#   awk '{OFS="\\t";if ($4+$5>=1) print $1,$2,$4,$5,$5/($4+$5)}' $singleC_sam_prefix/all.${query}.bed >$singleC_sam_prefix/all.${query}.1x-srt
#   ln -s $singleC_sam_prefix/all.${query}.1x-srt  /WPSnew/lilin/mouse_NOME_seq_select/StatInfo_steps/02.Tile/1x_singleC/$meth_name.${query}.1x-srt
#
...
#
#if [ $sc == "1" ]
##chr start end depth meth
#   then awk '{OFS="\\t";if ($4+$5==0) print $1,$2,$3,0,-1;if ($4+$5>0 && $5/($4+$5)<=0.1) print $1,$2,$3,$4+$5,0;if ($4+$5>0 && $5/($4+$5)>=0.9) print $1,$2,$3,$4+$5,1;if ($4+$5>0 && $5/($4+$5)>0.1 && $5/($4+$5)<0.9) print $1,$2,$3,$4+$5,-1}' $singleC_sam_prefix/all.${query}.bed >$singleC_sam_prefix/all.${query}.digital.bed
#   $bgzip_exe -c $singleC_sam_prefix/all.${query}.digital.bed > $singleC_sam_prefix/all.${query}.digital.bed.gz  &&\\
#   $tabix_exe -f -p bed -s 1 -b 2 -e 3  $singleC_sam_prefix/all.${query}.digital.bed.gz
#   awk '{OFS="\\t";if ($4<=0.1) print $1,$2,$3,0;if ($4>=0.9) print $1,$2,$3,1}' $singleC_sam_prefix/all.${query}.bedGraph >$singleC_sam_prefix/all.${query}.digital.bedGraph &&\\
#   $bgzip_exe -c $singleC_sam_prefix/all.${query}.digital.bedGraph > $singleC_sam_prefix/all.${query}.digital.bedGraph.gz &&\\
#$tabix_exe -f -p bed -s 1 -b 2 -e 3  $singleC_sam_prefix/all.${query}.digital.bedGraph.gz
#
#$ucsc_dir/bedGraphToBigWig                                             \\
#    $singleC_sam_prefix/all.${query}.digital.bedGraph                           \\
#    $sites_ref  $singleC_sam_prefix/all.${query}.digital.bw
#    awk '{OFS="\\t";if ($4+$5>=1 && $5/($4+$5)<=0.1) print $1,$2,$4,$5,0;if ($4+$5>=1 && $5/($4+$5)>=0.9) print $1,$2,$4,$5,1}' $singleC_sam_prefix/all.${query}.bed >$singleC_sam_prefix/all.${query}.1x-srt
#    ln -s $singleC_sam_prefix/all.${query}.1x-srt  /WPSnew/lilin/mouse_NOME_seq_select/StatInfo_steps/02.Tile/1x_singleC/$meth_name.${query}.1x-srt
#fi
       """)
        return l_sh_info

    def s_09_NUC_TSS_plot(self):
        l_sh_info = []
        l_sh_info.append("meth_name=$1")
        l_sh_info.append("query=GCA.GCC.GCT")
        l_sh_info.append("ref=mm9")
        l_sh_info.append("database=%s/$ref" % (self.Database))
        l_sh_info.append("singleC_dir=%s"   % (self.dir_singleC))
        l_sh_info.append("NDR_dir=%s"          % (self.dir_NDR))
        l_sh_info.append("plot_dir=%s"   % (self.dir_plot))
        l_sh_info.append("mergeSC_py=%s/merge_chrSC.py" %(self.bin))
        l_sh_info.append("bw_map_py=%s/DNase_plot_nuc_TSS/bw_map.py " %(self.bin))
        l_sh_info.append("plot_R=%s/DNase_plot_nuc_TSS/Meth_Signal_2Yaxes.R " %(self.bin))
        l_sh_info.append("bw_pl=%s/DNase_plot_nuc_TSS/bigWigSummary_TSS1k.pl" %(self.bin))
        l_sh_info.append("""
singleC_sam_prefix=$singleC_dir/$meth_name/singleC
sites_ref=$database/${ref}_lambda.fa.fai

python $mergeSC_py -r $sites_ref -c $query.nucleosome $singleC_sam_prefix &&\\
#python $mergeSC_py -r $sites_ref -c $query.NDR $singleC_sam_prefix &&\\
#ln -s $singleC_sam_prefix/all.$query.NDR.bed $NDR_dir/$meth_name.$query.NDR.bed &&\\
python $bw_map_py --ext_len 1000 --out_bin 100  $singleC_sam_prefix/all.GCA.GCC.GCT.nucleosome.bed $singleC_sam_prefix/all.GCA.GCC.GCT.bw >$singleC_sam_prefix/$meth_name.nuc.GCH.mat &&\\
python $bw_map_py --ext_len 1000 --out_bin 100  $singleC_sam_prefix/all.GCA.GCC.GCT.nucleosome.bed /datd/lilin/Project/NOM-Seq/Publised/01.DNaseI-Seq_MNase-Seq-signal/GSE69098_MNase.141_190.center_60.normalized.bw >$singleC_sam_prefix/$meth_name.nuc.MNase.mat &&\\
Rscript $plot_R $singleC_sam_prefix/$meth_name.nuc.GCH.mat $singleC_sam_prefix/$meth_name.nuc.MNase.mat $meth_name.nuc
perl $bw_pl $singleC_sam_prefix/all.GCA.GCC.GCT.bw |sed 's/n\/a/nan/g' >$singleC_sam_prefix/$meth_name.TSS.GCH.mat  &&\\
Rscript $plot_R $singleC_sam_prefix/$meth_name.TSS.GCH.mat /datd/lilin/Project/NOM-Seq/mouse_NOME_seq/StatInfo_steps/06.nucleosome/plot_with_MNase/plot_TSS/TSS_MNase2.mat $meth_name.TSS
       """)
        return l_sh_info
...
$bedtools_exe intersect -v -sorted                                          \\
    ...
    python $cut_NDR_support_dep_exe /dev/stdin                 \\
