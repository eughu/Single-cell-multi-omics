...
##        if np_obs[0] < np_exp[0]:    		 ### np_obs[0] "0" means unmet, only when unmeth ratio is lower than genomebackground to calculate pval
	if np_obs[0] > np_exp[0]:    ##only when unmeth ratio is higher than genome background to calculate pval, =>close region
...
#    CUTOFF = 1e-5 	#Revised by Lilin at 20150901
    ...
    for line in f_infile:
        ...
        if cnt_tot < DEPTH:
            continue
        if cnt_met/cnt_tot > 0.1 and cnt_met/cnt_tot <0.9: 
            continue    
