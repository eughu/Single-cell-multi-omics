...
class NDR(object):
#    def __init__(self, chrom, count_u, count_m, cutoff = 1e-5, bin_len=100, step_len=20, dist_len=140 ): 	#Revised by Lilin at 20150901
    def __init__(self, chrom, count_u, count_m, cutoff = 1e-15, bin_len=100, step_len=20, dist_len=140 ):
        self.chrom    = chrom
...
#        np_idx = np.array([ self.NDR_info['idx'].index(i) for i in l_val ],dtype="int")
	...
	np_total = np.sum(np_met) + np.sum(np_umt)
	np_meth_ratio = np.sum(np_met)/np_total					###add another colonme to print meth ratio
	np_length = self.NDR_info['end'] - self.NDR_info['beg'] +1
        ...
        print "%s\t%d\t%d\t%d\t%s\t%s\t%s\t%s" % (self.chrom, self.NDR_info['beg'], self.NDR_info['end'], np_length ,str_p, str_u, str_m , np_meth_ratio)  #####added by Lilin at 20150904
    ...
    def __chisq_bin(self, idx):
        ...
        '''
        if np_obs[0] < np_exp[0]:    		 ### np_obs[0] "0" means unmet, only when unmeth ratio is lower than genomebackground to calculate pval
...
def main():
    ...
    DEPTH = 3
#    CUTOFF = 1e-5 	#Revised by Lilin at 20150901
    CUTOFF = 1e-15
    ...
    for line in f_infile:
        ...
        if cnt_tot < DEPTH:
            continue
        if cnt_met/cnt_tot > 0.1 and cnt_met/cnt_tot <0.9:		##for single cell: only keep site with methRatio <=0.1 or >=0.9
            continue							##for single cell: only keep site with methRatio <=0.1 or >=0.9
