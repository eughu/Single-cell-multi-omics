...
int compMismtach(string s1, string s2){
        int cntMis = 0;
        string N_char = "N";
        for (int i=0; i<s1.size(); ++i){
                if(alphabet[s1[i]] == 4 or alphabet[s2[i]] == 4){
                        continue;
                }
                else{
                        if(s1[i] != s2[i]){
...
void usage()
{
        ...
        cout << "  -m  Max number for reporting a match"   << endl;
...
int main(int argc, char *argv[])
{
        ...
        while ( (c=getopt(argc,argv,"s:m:h")) != -1 ) {
                ...
                case 'm': maxMis = atoi(optarg); break;
                case 'h': usage(); break;
                default: usage();
                }
        }
        ...
        for (map_size=ChrLen.begin(); map_size!=ChrLen.end(); map_size++) {
                ...
                for (int i=0; i<len-max_len; i++) {
                        ...
                        for (int j=0; j<V_site.size(); j++) {
                                ...
                                int misNumber1 = compMismtach(tmp1, V_site[j]);
                                int misNumber2 = compMismtach(tmp2, V_site[j]);
                                if (misNumber1 <= maxMis) {
                                        outfile_bed << chr << "\t" << i+1 << "\t" << i+tmp1.size() << "\t" << "+" << "\t" << tmp1 << "\t" << V_site[j] << "\t";
                                        if (pre_site != 0) {
                                                outfile_bed << i - pre_site << "\t" << misNumber1 << endl;
                                                outfile_dist << i - pre_site << endl;
                                        }
                                        else{
                                                outfile_bed << 0 << "\t" << misNumber1 << endl;
                                        }
                                        pre_site = i;
                                }

                                if (misNumber2 <= maxMis) {
                                        outfile_bed << chr << "\t" << i+1 << "\t" << i+tmp2.size() << "\t" << "-" << "\t" << tmp2 << "\t"<< V_site[j] << "\t";
                                        if (pre_site != 0) {
                                                outfile_bed << i - pre_site << "\t" << misNumber2 << endl;
                                                outfile_dist << i - pre_site << endl;
                                        }
                                        else{
                                                outfile_bed << 0 << "\t" << misNumber2 << endl;
                                        }
                                        pre_site = i;
                                }
